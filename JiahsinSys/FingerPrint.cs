...
namespace FingerPrint
{
    public partial class FingerPrint : Form
    {
        ...
        public void Write_log(string t)
        {
           // string name  =DateTime.Now.ToString("yyyyMMdd");
            string folderLocation = Application.StartupPath + "\\log\\";
            bool folderExists = Directory.Exists(folderLocation);
            if (!folderExists)
            {
                Directory.CreateDirectory(Application.StartupPath + "\\log\\");
            }
             StreamWriter myFile = new StreamWriter(Application.StartupPath + "\\log\\" + t + ".txt", append: true);
             StringBuilder getdata = new StringBuilder();
             try
             {
                 for (int i = lbox_info.Items.Count-countline; i < lbox_info.Items.Count; i++)
                 {

                     getdata.Append(lbox_info.Items[i] + "\r\n");
                 }
